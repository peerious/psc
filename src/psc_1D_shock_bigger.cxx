...
#elif CASE == CASE_1D
  Grid_t::Real3 LL = {1., 16., 18000.}; // domain size (in d_e)
...
#if CASE == CASE_1D
  outf_params.pfield_interval = 1500;
  ...
  auto lf_inject = [&](int kind, Double3 pos, int p, Int3 idx,
    ...
    if (inject_target.is_inside(pos)) {
      ...
      if (kind == MY_ELECTRON_TARGET ) {
        npt.n =
          inject_target.n - mf_n(MY_ELECTRON_TARGET, idx[0], idx[1], idx[2], p)
	  - mf_n(MY_ELECTRON_AMBIENT, idx[0], idx[1], idx[2], p);
